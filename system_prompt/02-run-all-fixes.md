...
3. **Read Results:** For each fix prompt, refer to its corresponding result report located in the `system-prompt-qa-suite\result` directory (e.g., `system-prompt-qa-suite\result\result-Functional.md`) to understand the exact issues that need to be resolved.
