...
5. **Save Output:** Save the output into the `result` directory. The output filename must follow the convention `result-{Category}.md` (e.g., output of `prompts\check\Security.md` should be saved as `result\result-Security.md`).

**Constraint:** 
- Do NOT attempt to fix the codebase during this phase. 
- Only evaluate and generate result reports in the `result` directory.
